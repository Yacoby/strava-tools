...
import { Polyline, GeoJSON, TileLayer } from 'react-leaflet';
...
const editableMap = (Map) => {
  return class extends React.Component {
    ...
    crop() {
      this.props.onCropPressed &&
        this.props.onCropPressed(this.state.cropLines);
    }
      ...
      return (
        <div className='row'>
          <div className='col-lg-3'>
            <p>Click on the map to add crop start and end lines</p>
          ...
          <div className='col'>
            ...
            >
              {this.state.cropLines.map((line) => (
                <Polyline
                  positions={line}
                  color={this.getCropLineColor(line.isCropLineEnd)}
                />
              ))}
              {this.state.currentLineStart && (
                <Polyline
                  positions={[this.state.currentLineStart, this.state.mousePos]}
                  color={this.getCropLineColor(this.state.currentIsCropEnd)}
                />
              )}
            </Map>
          </div>
